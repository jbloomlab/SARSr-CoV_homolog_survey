...
Filter out mutant genotypes if they weren't sampled in both biological replicates, or at least 5 barcodes in the single replicate where observed. (Except the 'pool6' measures, which we only did in singles). This means we're not retaining any barcodes that don't show up in our correlation plots showing binding constants between the fully biological duplicates.
...
Last full data-dump heatmap to show -- normalize each heatmap so its illustrating delta-Ka relative to the *same* reference state in each sub-panel. Easiest is probably either the ancestral state in AncSarbecovirus, or the SARS-CoV-2 state... First, refernece state AncSarbecovirus state
