...
```{r unfiltered_expression_binding, echo=T, fig.width=10, fig.height=44, fig.align="center", dpi=300,dev="png"}
...
```{r plot_correlations, echo=T, fig.width=22, fig.height=5, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,9))
...
```{r plot_sterr_v_n, echo=T, fig.width=9, fig.height=9, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(3,3))
...
Collapse down to mean from both replicates, and total n barcodes between the two replicates. Also record the number of the replicates the variant was quantified within.
...
Filter out mutant genotypes if they weren't sampled in both biological replicates, or at least 5 barcodes in the single replicate where observed. (Except the 'pool6' measures, which we only did in singles).

```{r remove_nonduplicates}
...
```{r n_barcode_plots, echo=T, fig.width=32, fig.height=7, fig.align="center", dpi=300,dev="png"}
par(mfrow=c(2,12))
...
```{r heatmap_wildtypes_all, fig.width=12,fig.height=7,fig.align="center", dpi=500,dev="png",echo=T}
...
```
```{r heatmap_wildtypes_phylogeny, fig.width=12,fig.height=7,fig.align="center", dpi=500,dev="png",echo=T}
#make temp long-form data frame
...
```{r heatmap_wildtypes_MAP-ancestors, fig.width=12,fig.height=7,fig.align="center", dpi=500,dev="png",echo=T}
#make temp long-form data frame
...
Normalize each heatmap so its illustrating delta-Ka relative to the *same* reference state in each sub-panel so that comparisons can be made across a bit more easily (rather than each RBD having different reference states for the delta coloring). Easiest is probably either the ancestral state in AncSarbecovirus, or the SARS-CoV-2 state... First, refernece state AncSarbecovirus state
...
This is actually perhaps a useful zoomed in summary of the overwhelming heatmaps above. Let's do ligand-specific heatmaps in this arrangement. Which metric to show? First, white to purple heatmap showing affinities.
...
```{r heatmaps_all_SSM-by-bg-by-ACE2_affinity, fig.width=18,fig.height=45,fig.align="center", dpi=500,dev="png",echo=T}
...
```{r heatmaps_all_SSM-by-bg-by-ACE2_delta, fig.width=18,fig.height=45,fig.align="center", dpi=500,dev="png",echo=T}
