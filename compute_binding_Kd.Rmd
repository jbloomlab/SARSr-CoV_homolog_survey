...
For the huACE2 titration sorts, the fluorescence boundaries for bins 1-4 are as follows:
...
We successfully generated *K*<sub>D,app</sub> estimates for `r round(sum(!is.na(dt$Kd_huACE2))/nrow(dt)*100,digits=2)`% of our lib1+lib2 huACE2 titrations, `r round(sum(!is.na(dt$Kd_cvACE2))/nrow(dt)*100,digits=2)`% of our lib1+lib2 cvACE2 titrations, `r round(sum(!is.na(dt$Kd_pgACE2))/nrow(dt)*100,digits=2)`% of our lib1+lib2 pgACE2 titrations, `r round(sum(!is.na(dt$Kd_mACE2))/nrow(dt)*100,digits=2)`% of our lib1+lib2 mACE2 titrations, `r round(sum(!is.na(dt$Kd_RaACE2.787))/nrow(dt)*100,digits=2)`% of our lib1+lib2 RaACE2.787 titrations, `r round(sum(!is.na(dt$Kd_RaACE2.9479))/nrow(dt)*100,digits=2)`% of our lib1+lib2 RaACE2.9479 titrations, `r round(sum(!is.na(dt$Kd_RsACE2.3364))/nrow(dt)*100,digits=2)`% of our lib1+lib2 RsACE2.3364 titrations, and `r round(sum(!is.na(dt$Kd_RsACE2.1434))/nrow(dt)*100,digits=2)`% of our lib1+lib2 RsACE2.1434 titrations.
...
```{r Kd_distribution, fig.width=5, fig.height=24, fig.align="center",dpi=300,dev="png"}
par(mfrow=c(11,1))
...
Next, with *K*<sub>D,app</sub> around 10<sup>-6</sup>. These 10<sup>-6</sup> curves do seem to perhaps show binding signal beyond the 10<sup>-5</sup> fits? Previously we've collapsed this mode, but having the ultra-replicate barcoding scheme should actually help us out to pull out if there's any signal.
...
```{r nMSR_distribution, fig.width=8, fig.height=12, fig.align="center",dpi=300,dev="png"}
par(mfrow=c(4,3))
...
```{r final_pheno_DFE, fig.width=10, fig.height=36, fig.align="center",dpi=300,dev="png"}
...
Finally, let's output our measurements for downstream analyses. We'll output the normal Kds file for mainstream processing, but also output a file that has all of the meanbin and totalcount columns, in case we want to be able to re-visualize particular titration curves without having to run this entire script.
