...
Let's look at coverage of mutants at each position across each background. For assessing coverage of mutants in the different backgrounds, let's pool together the SARS-CoV-2 (genscript mutants) and SARS-CoV-2_2649 sequence (my in-house mutagenesis to add the missing mutants at position 494). Let's also only consider the SARS-CoV-1_2693 sequences, as these are what are actually pooled in our libraries for the titration experiments, as SARS-CoV-1_Urbani SSM library from Genscript arrived late for experiments, and I only pooled it here for PacBio sequencing in case my in-house mutagenesis was unsuccessful. (And it should therefore have zero counts in the Illumina sequencing, anyway, so fine to purge now for downstream counts management.)
